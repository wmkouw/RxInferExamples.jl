...
export OmniDriveBot, DifferentialDriveBot, update, emit

mutable struct OmniDriveBot
    ...
    function OmniDriveBot(ρ::Vector, σ::Vector; Δt::Float64=1.0, control_lims=(-1.,1.))
...
mutable struct DifferentialDriveBot
    ...
    function DifferentialDriveBot()
        return new()
    end
end

function step(bot::OmniDriveBot, z_kmin1, u_k)
...
function step(bot::DifferentialDriveBot, z_kmin1, u_k)

end

function emit(bot::OmniDriveBot, z_k)
    "Stochastic observation"

    return rand(MvNormal(bot.C*z_k, bot.R))
end

function update(bot::OmniDriveBot, z_kmin1, u_k)
    "Update environment" 
